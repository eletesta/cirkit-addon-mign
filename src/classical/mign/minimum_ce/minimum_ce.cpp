...
struct exact_mig_instance_min
{
 ...
 solver.add((var[x] == neg[0][i]) && (var[k] == neg[1][i]) && (var[j] == neg[2][i]));
  ...
  mign_graph extract_solution( const std::string& model_name, const std::string& output_name, bool verbose )
  {
    ...
    std::vector<mign_function> inputs, nodes;
    ...
    {
      std::vector<mign_function> children(3);
...
mign_graph exact_mig_ce_with_sat_explicit( const tt& spec, unsigned start, const std::string& model_name, const std::string& output_name, bool verbose)
{
    auto p = 0u;
    auto k = start;
    while ( true )
                                ...
                                const properties::ptr& statistics)
  {
    ...
    if ( minimum_after )
    {
       ...
       std::cout << format( "[i] run-time: %.2f seconds" ) % statistics->get<double>( "runtime" ) << std::endl;
  	  ...
  	  return exact_mig_ce_with_sat_explicit( spec, start, model_name, output_name,verbose);
